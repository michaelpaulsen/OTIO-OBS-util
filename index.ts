...
import {
    get_path_from_url,
    mime_type_from_file_extesion,
    normalize_url,
    read_file,
    stringify_ingore_priv
} from "./util.mjs";
...
obs.on("RecordStateChanged", async (e) => {
    ...
    if (e.outputState == 'OBS_WEBSOCKET_OUTPUT_STOPPED') {
        ...
        tl.tracks.children.forEach(element => {
            //we know that this is not null because we already check for null above! 
...
function add_marker_to_timeline(tl:Timeline , tp: Marker | RationalTime){ 
...
async function main(request: IncomingMessage, response: ServerResponse) {
    let url: URL = new URL(`http://${process.env.HOST ?? 'localhost'}${request.url}`);
